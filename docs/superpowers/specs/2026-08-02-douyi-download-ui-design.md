...
| FR4 | **类型字段**：下拉选择（ComboBox），选项在设置页增删管理；默认五个：中三、中四、平四、三步、其他。进入命名步时默认选中上次用过的类型。 |
...
- **命名步**：编号/类型/标题三字段，类型为下拉选择（选项在设置页管理）；原标题区支持鼠标圈字自动填入；两个并排大按钮（下载视频/下载音频）；取消为角落小字链接。
- **完成页**：大对勾 + "下载完成！" + 文件名 + 两个按钮；列表最新项置顶。
- **设置页**：保存位置、字体大小、类型选项（增删管理）、疑难解答、关于五个分区；左上角返回。
...
详见 `AGENTS.md` 决策记录。要点：WPF/.NET 技术栈；yt-dlp 引擎（曾评估 Evil0ctal/Douyin_TikTok_Download_API，因 Cookie 维护成本放弃）；`序号 类型 标题` 命名；编号三级默认值；向导 + 列表结合的界面；全 UI 无“家人”表述。类型字段改为下拉选择（ComboBox），选项在设置页增删管理；默认五个：中三、中四、平四、三步、其他。
